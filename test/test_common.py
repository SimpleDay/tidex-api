...
import tidexapi
from tidexapi.common import parseJSONResponse

# TODO: Add test for using Connection with a proxy.
...
class TestCommon(unittest.TestCase):
    ...
    def test_formatCurrency(self):
        self.assertEqual(tidexapi.formatCurrencyDigits(1.123456789, 1), "1.1")
        self.assertEqual(tidexapi.formatCurrencyDigits(1.123456789, 2), "1.12")
        self.assertEqual(tidexapi.formatCurrencyDigits(1.123456789, 3), "1.123")
        self.assertEqual(tidexapi.formatCurrencyDigits(1.123456789, 4), "1.1234")
        self.assertEqual(tidexapi.formatCurrencyDigits(1.123456789, 5), "1.12345")
        self.assertEqual(tidexapi.formatCurrencyDigits(1.123456789, 6), "1.123456")
        self.assertEqual(tidexapi.formatCurrencyDigits(1.123456789, 7), "1.1234567")

        for i in range(2, 8):
            print(i)
            self.assertEqual(tidexapi.formatCurrencyDigits(1.12, i), "1.12")
            self.assertEqual(tidexapi.formatCurrencyDigits(44.0, i), "44.0")

    def test_formatCurrencyByPair(self):
        info = tidexapi.APIInfo(self.connection)
        for i in info.pairs.values():
            d = i.decimal_places
            self.assertEqual(i.format_currency(1.12),
                             tidexapi.formatCurrencyDigits(1.12, d))
            self.assertEqual(i.format_currency(44.0),
                             tidexapi.formatCurrencyDigits(44.0, d))
            self.assertEqual(i.truncate_amount(1.12),
                             tidexapi.truncateAmountDigits(1.12, d))
            self.assertEqual(i.truncate_amount(44.0),
                             tidexapi.truncateAmountDigits(44.0, d))
    ...
    def test_validatePair(self):
        ...
        self.assertRaises(tidexapi.InvalidTradePairException,
    ...
    def test_validateOrder(self):
        ...
        for pair_name, pair_data in info.pairs.items():
            for t in ("buy", "sell"):
                ...
                self.assertRaises(tidexapi.InvalidTradeAmountException,
                                  info.validate_order, pair_name, t, pair_data.min_price,
                                  pair_data.min_amount - decimal.Decimal("0.0001"))
                self.assertRaises(tidexapi.InvalidTradePriceException,
